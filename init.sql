...
COPY public.movies (id,original_title,budget,original_language,revenue,runtime,genres)
    FROM '/processed-data/movies.csv'
    DELIMITER ','
    CSV HEADER;
...
COPY public.user_movies_temp (userId, movieId, rating, timestamp)
    FROM '/processed_data/ratings_small.csv'
    DELIMITER ','
    CSV HEADER;
...
COPY public.actors (id, name, gender)
    FROM '/processed_data/cast_info.csv'
    DELIMITER ','
    CSV HEADER;
...
COPY public.crew_temp (id, name, gender)
    FROM '/processed_data/crew_info.csv'
    DELIMITER ','
    CSV HEADER;
...
COPY public.acted_in (actor_id, movie_id, character)
    FROM '/processed_data/cast_movie_relationship.csv'
    DELIMITER ','
    CSV HEADER;
...
COPY public.crew_movie (actor_id, movie_id, department)
    FROM '/processed_data/crew_movie_relationship.csv'
    DELIMITER ','
    CSV HEADER;
